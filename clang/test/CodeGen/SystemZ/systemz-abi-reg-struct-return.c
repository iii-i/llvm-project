...
// Complex values are arithmetic types rather than composites, so the option
// does not affect them.

_Complex char ret_complex_char(void) { return 0; }
// CHECK-LABEL: define{{.*}} void @ret_complex_char(ptr dead_on_unwind noalias writable sret({ i8, i8 }) align 1 %{{.*}})
// MEMORY-LABEL: define{{.*}} void @ret_complex_char(ptr dead_on_unwind noalias writable sret({ i8, i8 }) align 1 %{{.*}})
...
// MEMORY-LABEL: define{{.*}} void @ret_complex_int(ptr dead_on_unwind noalias writable sret({ i32, i32 }) align 4 %{{.*}})
...
// MEMORY-LABEL: define{{.*}} void @ret_complex_float(ptr dead_on_unwind noalias writable sret({ float, float }) align 4 %{{.*}})

_Complex double ret_complex_double(void) { return 0; }
// CHECK-LABEL: define{{.*}} void @ret_complex_double(ptr dead_on_unwind noalias writable sret({ double, double }) align 8 %{{.*}})
...
// MEMORY-LABEL: define{{.*}} void @ret_complex_longdouble(ptr dead_on_unwind noalias writable sret({ fp128, fp128 }) align 8 %{{.*}})


// Vectors are returned in %v24 when the vector facility is available, and in
// memory when it is not; the option does not affect them either.

typedef __attribute__((vector_size(8))) int v2i32;
v2i32 ret_v2i32(void) { return (v2i32){ 0 }; }
// NO-VECTOR-LABEL: define{{.*}} void @ret_v2i32(ptr dead_on_unwind noalias writable sret(<2 x i32>) align 8 %{{.*}})
// VECTOR-LABEL: define{{.*}} <2 x i32> @ret_v2i32()
// MEMORY-LABEL: define{{.*}} void @ret_v2i32(ptr dead_on_unwind noalias writable sret(<2 x i32>) align 8 %{{.*}})

typedef __attribute__((vector_size(16))) int v4i32;
v4i32 ret_v4i32(void) { return (v4i32){ 0 }; }
// NO-VECTOR-LABEL: define{{.*}} void @ret_v4i32(ptr dead_on_unwind noalias writable sret(<4 x i32>) align 16 %{{.*}})
// VECTOR-LABEL: define{{.*}} <4 x i32> @ret_v4i32()
// MEMORY-LABEL: define{{.*}} void @ret_v4i32(ptr dead_on_unwind noalias writable sret(<4 x i32>) align 16 %{{.*}})
