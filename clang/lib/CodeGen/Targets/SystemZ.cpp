...
// are returned in %r2 and %r3 instead of in memory. Each register holds its
// part of the value in its low-order bits, just like a composite of the same
// size that is passed in an argument register.
std::optional<ABIArgInfo>
SystemZABIInfo::classifyCompositeReturnTypeInRegs(QualType Ty) const {
  if (!ReturnCompositesInRegs)
    return std::nullopt;

  // isCompoundType() also covers complex and vector types, which are not
  // aggregates and which the option does not affect.
